...
The first method I chose to classify the data with was quadratic discriminant analysis. This method incorporates [Bayes Theorem](https://brilliant.org/wiki/bayes-theorem/) to estimate $\Pr(Y = k{\mid}X = x)$, the probability of a digit being $k$ given that the observation's principal components looking the way they do. Bayes Theorem states that
\[
...
The demominator, $\Pr(X = x)$, can be estimated by 

and the variance being estimated by the for observations coming from classsing the sample proportion of each digit to estimate prior digit probabilities and, under the assumption that the data follows a multivariate Normal distribution, using sample means and covariance matrixes to estimate parameters. Whichever digit gives the highest probability is chosen.
...
Quadratic discriminant analysis predicted about `r kfoldpercentage*100`% of the digits correctly. 
